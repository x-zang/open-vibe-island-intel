...
- **Terminal.app**、**Ghostty**、**cmux**、**Kaku**、**WezTerm** 和 **iTerm2** — 完整的 jump-back 支持，带会话附着匹配（cmux 通过 Unix socket API，Kaku/WezTerm 通过 CLI pane 定位，iTerm2 通过 AppleScript session/TTY 探针）
- **Warp** — 降级检测和基本进程发现
